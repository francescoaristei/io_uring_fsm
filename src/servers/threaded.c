...
void *server_thread(void *arg) {
  ...
  int connected_sock = config->sockfd; // pass by copy
...
void serveSockWithThread(int connected_sock) {
  nclients++;
  pthread_t the_thread; // thread that will handle client request
  thread_config_t *config = (thread_config_t *)malloc(sizeof(*config)); // allocate memory for the struct
