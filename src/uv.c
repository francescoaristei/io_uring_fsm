...
#define N_BACKLOG 64 // number of connections the kernel queues
...
// method to send back an ack for benchmark
void send_back(uv_stream_t *client_stream) {
  client_state_t *c = get_client_state(client_stream);
  message_t m = c->curmsg;
  strcpy((char *)m.actions, "");

  // uv_buf_init =  Constructor for uv_buf_t.
  uv_buf_t writebuf = uv_buf_init((char *)m.actions, 1);
